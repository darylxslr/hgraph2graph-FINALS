...
class HierVAE(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.decoder = HierMPNDecoder(args.vocab, args.atom_vocab, args.rnn_type, args.embed_size, args.hidden_size, args.latent_size, args.diterT, args.diterG, args.dropout, device=args.device)
    ...
    def reconstruct(self, batch):
        ...
        tree_tensors, graph_tensors = tensors = make_cuda(tensors, self.device)
...
class HierVGNN(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.device = args.device
        self.encoder = HierMPNEncoder(args.vocab, args.atom_vocab, args.rnn_type, args.embed_size, args.hidden_size, args.depthT, args.depthG, args.dropout)
        self.decoder = HierMPNDecoder(args.vocab, args.atom_vocab, args.rnn_type, args.embed_size, args.hidden_size, args.hidden_size, args.diterT, args.diterG, args.dropout, attention=True, device=args.device)
        self.encoder.tie_embedding(self.decoder.hmpn)
    ...
    def translate(self, tensors, num_decode, enum_root, greedy=True):
        tensors = make_cuda(tensors, self.device)
        ...
        z_tree = torch.randn(batch_size, 1, self.latent_size).expand(-1, tree_vecs.size(1), -1).to(self.device)
        z_graph = torch.randn(batch_size, 1, self.latent_size).expand(-1, graph_vecs.size(1), -1).to(self.device)
    ...
    def forward(self, x_graphs, x_tensors, y_graphs, y_tensors, y_orders, beta):
        x_tensors = make_cuda(x_tensors, self.device)
        y_tensors = make_cuda(y_tensors, self.device)
...
class HierCondVGNN(HierVGNN):
    ...
    def translate(self, tensors, cond, num_decode, enum_root):
        ...
        z_tree = torch.randn(num_decode, 1, self.latent_size).expand(-1, tree_vecs.size(1), -1).to(self.device)
        z_graph = torch.randn(num_decode, 1, self.latent_size).expand(-1, graph_vecs.size(1), -1).to(self.device)
    ...
    def forward(self, x_graphs, x_tensors, y_graphs, y_tensors, y_orders, cond, beta):
        x_tensors = make_cuda(x_tensors, self.device)
        y_tensors = make_cuda(y_tensors, self.device)
        cond = torch.tensor(cond).float().to(self.device)
